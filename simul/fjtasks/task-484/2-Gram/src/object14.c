 ...
 * @file object14.c
 *
 * Port of the MRTC benchmark select
 */
/*************************************************************************/
/*                                                                       */
...
select(unsigned long k, unsigned long n)
{
	...
	int flag, flag2;
        ...
        float arr[20] = {5,   4,    10.3, 1.1,   5.7,   100,    231,
                         111, 49.5, 99,   10,    150,   222.22, 101,
                         77,  44,   35,   20.54, 99.99, 888.88};
        l=1;
	ir=n;
	flag = flag2 = 0;
	while (!flag) {
	  if (ir <= l+1) {
	    ...
	    mid=(l+ir) >> 1;
	    SWAP(arr[mid],arr[l+1]) // Panics the QEMU virt target
	      if (arr[l+1] > arr[ir]) {
		SWAP(arr[l+1],arr[ir])
		  }
	    if (arr[l] > arr[ir]) {
